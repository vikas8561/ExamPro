  ...
  if (timeLeft.completed) {
    return (
      <div className="flex items-center justify-center gap-2 text-[#34D399] font-semibold text-xs text-center bg-emerald-500/10 py-2.5 px-3 rounded-xl border border-emerald-500/20">
        ...
        <span>Test is now available to start!</span>
      </div>
    );
  }
...
const StudentAssignments = () => {
  ...
  // Summary counts for every assignment, not just the page on screen - the
  // server sends them because only it can see the whole set.
  ...
  // The poll and the socket refresh both call fetchAssignments from callbacks
  // created on mount, so reading the filters through refs keeps a background
  // refresh from quietly replacing a filtered list with an unfiltered one.
  ...
  useEffect(() => {
    const now = Date.now();
    const shouldFetch = now - lastFetchTime > 2000;
    ...
    socket.on("assignmentCreated", (data) => {
      console.log("Socket event: assignmentCreated", data);
      setLastFetchTime(0);
  ...
  // Re-query whenever a filter changes, always from page 1 -- the result set is
  // a different size, so staying on page 4 could land past its end. Typing is
  // debounced so a search is one request rather than one per keystroke; the
  // pills and the dropdown apply at once.
  ...
  const fetchAssignments = async (retryCount = 0, page = currentPage, forceRefresh = false) => {
    ...
    try {
      ...
      // The filters go to the server, which is the only place that can see
      // every assignment. Filtering here narrowed one page of nine, so a search
      // missed matches on other pages and a status pill disagreed with its tile.
      const params = new URLSearchParams({ page: String(page), limit: "9", exclude: "coding" });
      if (statusFilterRef.current !== "all") params.set("status", statusFilterRef.current);
      if (subjectFilterRef.current !== "all") params.set("subject", subjectFilterRef.current);
      if (searchTermRef.current.trim()) params.set("search", searchTermRef.current.trim());

      const data = await apiRequest(`/assignments/student?${params.toString()}`, {
  ...
  // Already filtered and sorted by the query, over every assignment rather than
  // the nine on screen.
  const filteredAssignments = assignments;

  // Calculate status counts
  // These describe every assignment the student has, so they come from the
  // server. Counting the rows in `assignments` tallied one page of nine, which
  // made the four tiles change every time the student turned the page and never
  // add up to the total shown beside them. The fallback only matters for the
  ...
  return (
    <div className="student-assignments-mobile min-h-screen bg-[#16181F] text-slate-100 font-sans p-6 lg:p-6 relative">
      <style>{`
      ...
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#7E8594] mt-0.5">
                {totalItems > 0 ? `${totalItems} official assessments assigned` : `${assignments.length} assignments`} • Timed evaluations
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2.5 flex-wrap sm:flex-nowrap">
            ...
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all active:scale-95 shadow-sm flex-shrink-0 ${
                showFilters || statusFilter !== "all" || subjectFilter !== "all"
        ...
        {(showFilters || statusFilter !== "all" || subjectFilter !== "all") && (
          <div className="p-4 rounded-2xl bg-[#181A22] border border-white/[0.06] shadow-sm space-y-3 transition-all">
            {/* Status Pills */}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-white/[0.04]">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {/* From the subject list, not from the rows on screen: once the
                    server does the filtering, choosing a subject would leave
                    that subject as the only button and no way back to another. */}
                {Array.from(
                  new Set(
                    (subjects.length
                      ? subjects.map((s) => s?.name || s)
                      : assignments.map((a) => a.testId?.subject)
                    ).filter(Boolean)
                  )
                ).map((subj) => (
                  <button
        ...
        {loading ? (
          ...
          <div className="rounded-2xl bg-[#181A22] border border-white/[0.05] p-12 text-center">
            ...
            <h3 className="text-base font-semibold text-white mb-1">No Tests Assigned</h3>
            <p className="text-xs text-[#7E8594] max-w-sm mx-auto">
              Any official exams or assessments assigned to your cohort will appear here.
            </p>
          </div>
          ...
          <div className="rounded-2xl bg-[#181A22] border border-white/[0.05] p-12 text-center">
            ...
            <p className="text-xs text-[#7E8594] mb-4">
              Try adjusting your search terms or clearing status/subject filters.
            </p>
            <button
              onClick={() => {
                setStatusFilter("all");
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredAssignments.map((assignment) => {
              const testTitle = assignment.testId?.title || "Untitled Test";
                ...
                >
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-5">
                      ...
                      <div className="flex items-center gap-2 p-2 rounded-xl bg-[#20242D]/60 border border-white/[0.03]">
                        <HelpCircle className="w-3.5 h-3.5 text-[#A78BFA] flex-shrink-0" />
                        <span className="text-xs text-slate-300 truncate font-medium">
                  ...
                  {/* Actions Row with STRICT Button Colors */}
                  <div className="pt-3 border-t border-white/[0.05] space-y-2">
                    {/* Countdown for Not Started Upcoming Test */}
